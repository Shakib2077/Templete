        ...
        <div class="row profile-body">
          ...
          <div class="col-md-12 col-xl-8 middle-wrapper">
			...
			<form method="POST" action="{{ route('admin.roles.update',$role->id) }}" class="forms-sample">
				...
				<div class="mb-3">
					<label for="exampleInputName" class="form-label">Roles Name</label>
                    <h3>{{ $role->name }}</h3>
				</div>
...
<hr>
                ...
                <div class="form-check mb-2">
                    <input type="checkbox" class="form-check-input" name="permission[]" id="checkDefault{{ $permission->id }}" value="{{ $permission->id }}" {{ $role->permissions->contains($permission->id) ? 'checked' : '' }}>
